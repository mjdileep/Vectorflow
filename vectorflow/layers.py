...
class Linear(Layer):
    ...
    def forward(self, x, **kwargs):
        ...
        """
        out = x.dot(self.params["w"])+ (self.params["b"] if self.bias else 0.0)
        self.cache = x
        return out
    ...
    def backward(self, dx):
        ...
        x = self.cache
        if x.ndim>2:
            grad  = np.transpose(x, (0, -1, -2)) @ dx
            grad = grad.sum(axis=0)
        else:
            grad  = x.T @ dx
        self.grads["w"] += grad
        if self.bias:
            self.grads["b"] += np.sum(dx, axis=tuple(range(dx.ndim - 1)))
...
class Norm(Layer):
    ...
    def norm(self, x):
        ...
        x_c = x- sample_mean
        x_d = x_c**2
        x_e = np.sum(x_d, axis=0, keepdims=True)
        sample_var = x_e/n
    ...
    def dnorm(self, dnorm):
        ...
        dnorm_sample_var = np.sum(dnorm_std*dstd_sample_var, axis=0, keepdims=True)
        dsample_var_x_e = 1/n
        ...
        dnorm_sample_mean =  np.sum(dnorm_x_c * dx_c_sample_mean-dnorm_dx_a,axis=0, keepdims=True)
...
class Layernorm(Norm):
    ...
    def __init__(self, size, momentum=0.9, eps=1e-5):
        super().__init__(eps)
        self.params["gamma"] = np.ones((1, size))
        self.grads["gamma"] = np.zeros((1, size))
        self.param_configs["gamma"] = {}

        self.params["beta"] = np.zeros((1, size))
        self.grads["beta"] = np.zeros((1, size))
        self.param_configs["beta"] = {}

    def forward(self, x, **kwargs):
        x_T = x.T
        norm, _, _ = self.norm(x_T)
        out = norm.T#*self.params["gamma"] + self.params["beta"]
        return out

    def backward(self, dx):
        _, _, _, _, norm, _, _, _, _, _ = self.cache
        self.grads["beta"] += np.mean(dx, axis=tuple(range(dx.ndim-1)), keepdims=1).reshape(1, -1)
        self.grads["gamma"] += np.mean((norm.T*dx), axis=tuple(range(dx.ndim-1)), keepdims=1).reshape(1, -1)
        
        dnorm = dx#*self.params["gamma"]
        dnorm_dx_1, dnorm_dx_2, dnorm_dx_3 = self.dnorm(dnorm.T)

        return dnorm_dx_1.T + dnorm_dx_2.T + dnorm_dx_3.T
